...
// Intermediate Level: Focus on identifying LONG vowel sounds
const WORDS = [
  { word: "cake", isCorrect: true },      // Long A
  { word: "kite", isCorrect: true },      // Long I
  { word: "rope", isCorrect: true },      // Long O
  { word: "cube", isCorrect: true },      // Long U
  { word: "team", isCorrect: true },      // Long E
  { word: "rain", isCorrect: true },      // Long A
  { word: "boat", isCorrect: true },      // Long O
  { word: "seed", isCorrect: true },      // Long E
  { word: "bike", isCorrect: true },      // Long I
  { word: "mule", isCorrect: true },      // Long U
  
  { word: "cat", isCorrect: false },      // Short A
  { word: "sit", isCorrect: false },      // Short I
  { word: "pot", isCorrect: false },      // Short O
  { word: "cup", isCorrect: false },      // Short U
  { word: "bed", isCorrect: false },      // Short E
  { word: "bag", isCorrect: false },      // Short A
  { word: "log", isCorrect: false },      // Short O
  { word: "fin", isCorrect: false },      // Short I
  { word: "bus", isCorrect: false },      // Short U
  { word: "pen", isCorrect: false },      // Short E
];

export default function IntermediateVowelGame() {
  ...
  useEffect(() => {
    if (isGameOver && !scoreSaved) {
      saveScoreToFirebase(score)
        .then(() => {
          console.log("Intermediate level score saved successfully");
          setScoreSaved(true);
        })
        .catch(error => {
  ...
  useEffect(() => {
    ...
    }, 1200); // Slightly faster than beginner level
  ...
  useEffect(() => {
    ...
    const gameLoop = setInterval(() => {
      setFallingWords((prev) =>
        prev
          .map((w) => ({ ...w, y: w.y + 12 })) // Faster falling speed
      ...
      <Modal visible={instructionsVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.instructionsBox}>
            <Text style={styles.instructionsTitle}>Intermediate Level</Text>
            <Text style={styles.instructionsSubtitle}>Long Vowel Sounds</Text>
            
            <View style={styles.instructionsContent}>
              <Text style={styles.instructionsText}>
                🎯 <Text style={styles.bold}>Goal:</Text> Catch words with LONG vowel sounds
              </Text>
              <Text style={styles.instructionsText}>
                ✅ <Text style={styles.bold}>Correct:</Text> cake, kite, rope, team, rain
              </Text>
              <Text style={styles.instructionsText}>
                ❌ <Text style={styles.bold}>Avoid:</Text> cat, sit, pot, bed, bus
              </Text>
              <Text style={styles.instructionsText}>
